...
import org.video.imusictube.extractor.utils.DashMpdParser;
...
public class StreamInfo extends Info {
    ...
    private static StreamInfo extractStreams(StreamInfo streamInfo, StreamExtractor extractor)
        ...
        try {
            ...
            streamInfo.addError(new ExtractionException("Couldn't get audio stream", e));
        }
        /* Extract video stream url */
        try {
            streamInfo.setVideoStreams(extractor.getVideoStreams());
        } catch (Exception e) {
            streamInfo.addError(new ExtractionException("Couldn't get video stream", e));
        }
        /* Extract video only stream url */
        try {
            streamInfo.setVideoOnlyStreams(extractor.getVideoOnlyStreams());
        } catch (Exception e) {
            streamInfo.addError(new ExtractionException("Couldn't get video only stream", e));
        }
