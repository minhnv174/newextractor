...
public class YoutubeChannelExtractor extends ChannelExtractor {
    ...
    public InfoItemsPage<StreamInfoItem> getPage(String pageUrl) throws IOException, ExtractionException {
        ...
        // Unfortunately, we have to fetch the page even if we are only getting next stream,
        ...
        JsonObject ajaxJson;
        try {
            ajaxJson = JsonParser.object().from(MusicTube.getDownloader().download(pageUrl));
        } catch (JsonParserException pe) {
            throw new ParsingException("Could not parse json data for next stream", pe);
        }
    ...
    private String getNextPageUrlFrom(Document d) throws ParsingException {
        try {
            ...
            if (button != null) {
                ...
                // Sometimes channels are simply so small, they don't have a more stream/videos
                return "";
            }
        } catch (Exception e) {
